  ...
  return (
    <>
      ...
      >
        ...
        {payload?.length <= 5 && !disableEdit && (
          <AddRow onClick={() => addProduct()} />
        )}
      </Page>
      {payload?.length > 5 && (
        <Page
...
const ProductRow = ({
  ...
  return (
    <Row key={index}>
      ...
      {!disableEdit && <DeleteCol onClick={() => deleteRow(index)} />}
    </Row>
  );
};
